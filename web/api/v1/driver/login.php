...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && !empty($password)) {
        ...
        // Query the database based on email and active status
        ...
        if ($result && mysqli_num_rows($result) == 1) {
            ...
            if (password_verify($password, $row['password'])) {
                // Strict Role Validation for Driver
                if ($row['role'] !== 'DR') {
                    $response['status'] = 'error';
                    $response['message'] = 'Unauthorized. Not a driver account.';
                ...
                $_SESSION['assigned_vehicle'] = $row['assigned_vehicle'];
