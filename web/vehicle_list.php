...
.dataTable th.sorting:after, .dataTable th.sorting_desc:before {
    top: 16px!important;
    right: 3px;
}
.dataTable th.sorting:before, .dataTable th.sorting_asc:after {
...
<div class="container-fluid main-content new_dpt_bottom">
								...
								<tr>
									...
									<td class="actions center-content ">
										<div class="action-buttons">
											<a title="Edit" href="vehicle.php?key=<?php echo md5($row['vehicle_id']); ?>" class="table-actions btn-edit" id="<?php echo $row['vehicle_id']; ?>"><i class="fa fa-pencil"></i></a>
											<?php
											if ($row['status'] == 0) {
												?>
											<a class="table-actions btn-active" data-status="<?php echo $row['status'] ?>" title="InActive" id="<?php echo $row['vehicle_id'] ?>"><i class="fa fa-check"></i></a>
											<?php
											} else {
											?>
											...
											?>
											<a title="Delete" href="#myModal" class="table-actions btn-trash" data-toggle="modal" id="<?php echo $row['vehicle_id'] ?>"><i class="fa fa-trash-o"></i></a>
		...
		$(document).ready(function(){
			...
			$(document).on('click', '.btn-confirm-delete', function(ev){
				...
				$.post('save_details.php', { form_name: "del_vehicle", tbl_id: $(this).attr("id") }, function(data,status){	
				console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
						$("#alert-message").text("Vehicle Deleted successfully...");
						...
						$("#alert-message").text("Vehicle deletion failed");
		...
		//Active Inactive
			$(document).on('click', '.btn-active', function(ev){
				...
				$.post('save_details.php', { form_name: "inacv_vehicle", tbl_id: $(this).attr("id"),status:status1}, function(data,status){
					console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
						$("#alert-message").text("Vehicle  Is "+msg+"...");
						$("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function(){
						$("#alert-container").hide();
						$("#alert-container").removeClass("alert-success");
							location.reload();
