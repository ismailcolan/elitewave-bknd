                        ...
                        <div class="widget-content padded clearfix new_dept">
                            <table class="table table-bordered table-striped consignment_mode_tbl" id="dataTable1">
                                <thead>
                                    ...
                                    <th class="table-title" style="width:30%">Consignment Mode</th>
                                    <th class="table-title" style="width:20%">Description</th>
                                    <th class="table-title" style="width:20%">Mode of pay</th>
                                    <th class="table-title" style="width:10%">Action</th>
                                </thead>
                                <tbody>
                                    <?php
                                    $query = 'select * from consignment_mode';
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $row['consignment_mode']; ?></td>
                                            <td><?php echo $row['description']; ?></td>
                                            <td><?php echo $row['mode_of_pay']; ?></td>

                                            <td class="actions center-content ">
                                                <div class="action-buttons">
                                                    <a title="Edit" class="table-actions btn-edit" id="<?php echo $row['consignment_id']; ?>"><i class="fa fa-pencil"></i></a>
                                                    <?php
                                                    if ($row['status'] == 0) {
                                                        ?>
                                                        <a class="table-actions btn-active" data-status="<?php echo $row['status'] ?>" title="InActive" id="<?php echo $row['consignment_id'] ?>"><i class="fa fa-check"></i></a>
                                                    <?php
                                                    } else {
                                                        ?>
                                                        <a class="table-actions btn-active" style="color:red;" data-status="<?php echo $row['status'] ?>" title="Active" id="<?php echo $row['consignment_id'] ?>"><i class="fa fa-times"></i></a>
                                                    <?php
                                                    }
                                                    ?>
                                                    <a title="Delete" href="#myModal" class="table-actions btn-trash" data-toggle="modal" id="<?php echo $row['consignment_id'] ?>"><i class="fa fa-trash-o"></i></a>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                function duplicate_check() {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            console.log(result);
                            if (result[0] == 1) {
                                $(".consignment-error").html(result[1]).css("color", "#f00");
                                dup_chk = false;
                            } else {
                                $(".consignment-error").html(result[1]).css("color", "green");
                            }
                        },
                ...
                $(document).on('click', '#save', function() {
                    ...
                    duplicate_check();
                    if ($('#consignment_form').valid() == true && dup_chk) {
                ...
                $(document).on('click', '.btn-confirm-delete', function(ev) {
                    ...
                    $.post('save_details.php', {
                        ...
                        if (data == 1) {
                            ...
                            $(".form-data-saving").hide();
                            $("#alert-status").text("Alert !!! ");
                            $("#alert-message").text("Consignment deletion failed");
                            $("#alert-container").addClass("alert-danger").slideDown(800).fadeTo(1000, 500).slideUp(800, function() {
                                $("#alert-container").hide();
                                $("#alert-container").removeClass("alert-danger");
                            });
                ...
                $(document).on('click', '.btn-active', function(ev) {
                    ...
                    $.post('save_details.php', {
                        ...
                        if (data == 1) {
                            $(".form-data-saving").hide();
                            $("#alert-status").text("");
                            $("#alert-message").text("Consignment Is " + msg + "...");
                            $("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function() {
                                $("#alert-container").hide();
                                $("#alert-container").removeClass("alert-success");
                                location.reload();
                ...
                $(document).on('click', '.btn-edit', function(ev) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            $("#edit_id").val(result['consignment_id']);
