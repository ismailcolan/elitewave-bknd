		...
		<div class="row">
		  <div class="col-md-3 master_left">
			<div class="widget-container fluid-height clearfix">
			  <div class="heading"> <i class="fa fa-plus"></i>role Master</div>
			  
			  <div class="widget-content padded">
				<form class="form-horizontal" id="role_form">
				
					<input type="hidden" id="form_name" name="form_name" value="add_role">
				  ...
				  <div class="row">
						<div class="col-md-12">
							<div class="form-group">
								...
								<input type="text" id="role_name" name="role_name" placeholder="Role Name" class="form-control" required/>
							</div>
						</div>
				 </div><br/>
				   <div class="row">
		  ...
		  <div class=" col-md-9 master_right">
					<div class="widget-container fluid-height clearfix">
						<div class="heading"> <i class="fa fa-table" ></i> List of Roles </div>
					<div class="widget-content padded clearfix new_dept">
						<table class="table table-bordered table-striped" id="dataTable1">
							<thead>
								<th class="table-title" style="width:10%">S.No</th>
								<th class="table-title" style="width:10%">Role Name</th><th class="table-title" style="width:10%">Role</th>
								...
								<tr>
									...
									<td class="actions center-content ">
										<div class="action-buttons">
											<a title="Edit" class="table-actions btn-edit" id="<?php echo $row['role_id']; ?>"><i class="fa fa-pencil"></i></a>
											<?php 
											if($row['status'] == 0)
											{
											?>
											<a class="table-actions btn-active" data-status="<?php echo $row['status']  ?>" title="InActive" id="<?php echo $row['role_id'] ?>"><i class="fa fa-check"></i></a>
											<?php 
											}
											else
											{
											?>
											<a class="table-actions btn-active" style="color:red;" data-status="<?php echo $row['status']  ?>" title="Active" id="<?php echo $row['role_id'] ?>"><i class="fa fa-times"></i></a>
											<?php 
											}
											?>
											<a title="Delete" href="#myModal" class="table-actions btn-trash" data-toggle="modal" id="<?php echo $row['role_id'] ?>"><i class="fa fa-trash-o"></i></a>
	...
	$(document).on('keypress', '#journey_hours', function(evt){
		...
		//Button Delete
			...
			$(document).on('click', '.btn-confirm-delete', function(ev){
				...
				$.post('save_details.php', { form_name: "del_role", tbl_id: $(this).attr("id") }, function(data,status){	
				console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
						$("#alert-message").text("Role Deleted successfully...");
						$("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function(){
							$("#alert-container").hide();
							$("#alert-container").removeClass("alert-success");
							location.reload();
						...
						$(".form-data-saving").hide();
						$("#alert-status").text("Alert !!! ");
						$("#alert-message").text("Role deletion failed");
						$("#alert-container").addClass("alert-danger").slideDown(800).fadeTo(1000, 500).slideUp(800, function(){
							$("#alert-container").hide();
							$("#alert-container").removeClass("alert-danger");
						});
		...
		//Active Inactive
			$(document).on('click', '.btn-active', function(ev){
				...
				$.post('save_details.php', { form_name: "inacv_role", tbl_id: $(this).attr("id"),status:status1}, function(data,status){
					console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
						$("#alert-message").text("Role Is "+msg+"...");
						$("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function(){
						$("#alert-container").hide();
						$("#alert-container").removeClass("alert-success");
							location.reload();
			...
			$(document).on('click', '.btn-edit', function(ev){
				...
				$.ajax({
					...
					type : "GET", // type of action POST || GET
					dataType : 'json', // data type
					data : { cmd: "get_role_details", tbl_id: tbl_id }, // post data || get data
					success : function(result) {
					console.log(result);
						$(".form-data-saving").hide();
						$("#form_name").val("edit_role");
						$("#edit_id").val(result['role_id']);
