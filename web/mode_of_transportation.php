                        ...
                        <div class="widget-content padded clearfix new_dept">
                            <table class="table table-bordered table-striped mode_tab" id="dataTable1">
                                <thead>
                                    ...
                                    <th class="table-title" style="width:30%">Mode Type</th>
                                    <th class="table-title" style="width:20%">Delivery (Hrs)</th>
                                    <th class="table-title" style="width:20%">SAC Code</th>
                                    <th class="table-title" style="width:10%">Action</th>
                                </thead>
                                <tbody>
                                    <?php
                                    $query = 'select * from mode_of_transportation';
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $row['max_hrs_delivery']; ?></td>
                                <td><?php echo $row['sac_code']; ?></td>
                                            <td class="actions center-content ">
                                                <div class="action-buttons">
                                                    <a title="Edit" class="table-actions btn-edit" id="<?php echo $row['mode_id']; ?>"><i class="fa fa-pencil"></i></a>
                                                    <?php
                                                    if ($row['status'] == 0) {
                                                        ?>
                                                        <a class="table-actions btn-active" data-status="<?php echo $row['status'] ?>" title="InActive" id="<?php echo $row['mode_id'] ?>"><i class="fa fa-check"></i></a>
                                                    <?php
                                                    } else {
                                                        ?>
                                                        <a class="table-actions btn-active" style="color:red;" data-status="<?php echo $row['status'] ?>" title="Active" id="<?php echo $row['mode_id'] ?>"><i class="fa fa-times"></i></a>
                                                    <?php
                                                    }
                                                    ?>
                                                    <a title="Delete" href="#myModal" class="table-actions btn-trash" data-toggle="modal" id="<?php echo $row['mode_id'] ?>"><i class="fa fa-trash-o"></i></a>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $(document).on('click', '#save', function() {
                    var data = $('#transportation_form').serialize();
                    duplicate_check();
                    if ($('#transportation_form').valid() == true && dup_chk) {
                ...
                $(document).on('click', '.btn-confirm-delete', function(ev) {
                    ...
                    $.post('save_details.php', {
                        ...
                        if (data == 1) {
                            $(".form-data-saving").hide();
                            $("#alert-status").text("");
                            $("#alert-message").text("Mode Deleted successfully...");
                            $("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function() {
                                $("#alert-container").hide();
                                $("#alert-container").removeClass("alert-success");
                                location.reload();
                            ...
                            $(".form-data-saving").hide();
                            $("#alert-status").text("Alert !!! ");
                            $("#alert-message").text("Mode deletion failed");
                            $("#alert-container").addClass("alert-danger").slideDown(800).fadeTo(1000, 500).slideUp(800, function() {
                                $("#alert-container").hide();
                                $("#alert-container").removeClass("alert-danger");
                            });
                ...
                $(document).on('click', '.btn-active', function(ev) {
                    ...
                    $.post('save_details.php', {
                        ...
                        if (data == 1) {
                            $(".form-data-saving").hide();
                            $("#alert-status").text("");
                            $("#alert-message").text("Mode Is " + msg + "...");
                            $("#alert-container").addClass("alert-success").slideDown(800).fadeTo(1000, 500).slideUp(800, function() {
                                $("#alert-container").hide();
                                $("#alert-container").removeClass("alert-success");
                                location.reload();
