...
if (mysqli_num_rows($check_frq) > 0) {
    while ($row = mysqli_fetch_assoc($check_frq)) {
        ...
        // $frq_end = date('d-m-Y', strtotime($frq_date."+$inv_frq days"));
        // $frq_end = "13-10-2023";

        $query = 'SELECT * FROM '.$table[0].' WHERE STR_TO_DATE(grn_date,"%d-%m-%Y") < STR_TO_DATE("'.$current_date.'","%d-%m-%Y") AND  consigner = '.$client_id.' AND `mode_of_consignment` = 2 AND `status` = 8 AND `frq_sent_status` = "" AND (ISNULL(paid_status) OR `paid_status` = 0 OR `paid_status` = 2) AND `book_manual` = 1';
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            if ($count_files > 0) {
                $freq_type = $frq_day."D";
                $check_date_exist = mysqli_query($conn, "SELECT * FROM `automation_mail_report` WHERE consigner = '$consigner' AND frequency_type = '$freq_type' AND client_type ='1' AND `job_status` != 'SUCCESS'");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `frq_sent_date`,`created_at`, `updated_at`) VALUES('$freq_type','1','$consigner','$consignees','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued', '$current_date','$created_date','$created_date')";
                    ...
                    $queue_job_update = "UPDATE `automation_mail_report` SET `frequency_type`='$freq_type',`consignee`='$consignees',`grn_no`='$grn_numbers',`grn_date`='$grn_dates',`origin`='$origins',`destination`='$destinations',`transaction_id`='$transct_id',`invoice_amount`='$amounts',`invoice_no`='$invoice_no',`invoice_files`='$invoice_filess', `frq_sent_date`='$current_date',`updated_at`='$created_date' WHERE `consigner` = '$consigner'AND frequency_type = '$freq_type' AND client_type ='1' AND `job_status` != 'SUCCESS'";
	...
	// Below code is to fetch the data from previous quarter if quarter is changed means the table is also changed for that below code helps to get prev quarter table data    
        $query1 = 'SELECT * FROM '.$table_prev_quarter[0].' WHERE STR_TO_DATE(grn_date,"%d-%m-%Y") < STR_TO_DATE("'.$current_date.'","%d-%m-%Y") AND  consigner = '.$client_id.' AND `mode_of_consignment` = 2 AND `status` = 8 AND `frq_sent_status` = "" AND (ISNULL(paid_status) OR `paid_status` = 0 OR `paid_status` = 2) AND `book_manual` = 1';
        $transactions1 = mysqli_query($conn, $query1);
        $rest1 = mysqli_num_rows($transactions1);
        print_r($rest1);
        // exit();

        if (mysqli_num_rows($transactions1) > 0) {
            $get_transaction_id1 = [];
            $grn_no1 = [];
            $grn_date1 = [];
            $consignee1 = [];
            ...
            while ($row2 = mysqli_fetch_assoc($transactions1)) {
                ...
                $find_invoice_file1 = array($consigner1, $m_year_quarter . "_" . $row2['transaction_id'] . "invoice");
                if (in_array($consigner1, $find_invoice_file1)) {
            ...
            $consignees1 = implode(',', $consignee1);
            ...
            if ($count_files1 > 0) {
                $freq_type = $frq_day."D";
                $check_date_exist1 = mysqli_query($conn, "SELECT * FROM `automation_mail_report` WHERE consigner = '$consigner1' AND frequency_type = '$freq_type' AND client_type ='1' AND `job_status` != 'SUCCESS'");
                $result_check1 = mysqli_num_rows($check_date_exist1);

                if ($result_check1 == 0) {
                    $queue_job1 = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `frq_sent_date`,`created_at`, `updated_at`) VALUES('$freq_type','1','$consigner1','$consignees1','$grn_numbers1','$grn_dates1','$origins1','$destinations1','$transct_id1','$amounts1','$invoice_no1','$invoice_filess1','Queued', '$current_date','$created_date','$created_date')";
                    $ress1  = mysqli_query($conn, $queue_job1);
                    if ($ress1) {
                        $update_trans_table1 = "UPDATE $table_prev_quarter[0] SET `frq_sent_status`='Queued' WHERE consigner = '$client_id' AND `transaction_id` IN($transct_id1)";
                        $res_upd_trans_tbl1 = mysqli_query($conn, $update_trans_table1);
                        
                        echo "Queued Job Successfully";
                    }
                }else{
                    $queue_job_update1 = "UPDATE `automation_mail_report` SET `frequency_type`='$freq_type',`consignee`='$consignees1',`grn_no`='$grn_numbers1',`grn_date`='$grn_dates1',`origin`='$origins1',`destination`='$destinations1',`transaction_id`='$transct_id1',`invoice_amount`='$amounts1',`invoice_no`='$invoice_no1',`invoice_files`='$invoice_filess1', `frq_sent_date`='$current_date',`updated_at`='$created_date' WHERE `consigner` = '$consigner1'AND frequency_type = '$freq_type' AND client_type ='1' AND `job_status` != 'SUCCESS'";
                    $ress  = mysqli_query($conn, $queue_job_update1);
                    // echo "Data Already Exist";
                }
            } else {
