...
echo $find_client = "select * from client where invoice_frequency = '7'"; // 1000 Entry
    ...
    while ($row = mysqli_fetch_assoc($check_frq)) {
                ...
                if ($weeks[$i][$j + 1] != '') {
                    ...
                    if ($day == '01') {
                        ...
                        if ($current_date >= $weeks[$i][$j] && $current_date <= $weeks[$i][$j + 1]) {
                            ...
                            echo $query = 'SELECT * FROM ' . $table[0] . ' WHERE consigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2  AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $weeks[$i][$j] . '","%Y-%m-%d") and str_to_date("' . $weeks[$i][$j + 1] . '","%Y-%m-%d")';
                        }
                    } else if ($day == '08') {
                        ...
                        if ($current_date >= $weeks[$i][$j] && $current_date <= $weeks[$i][$j + 1]) {
                            ...
                            echo $query = 'SELECT * FROM ' . $table[0] . ' WHERE consigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2  AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $weeks[$i][$j] . '","%Y-%m-%d") - ' . $days . ' and str_to_date("' . $weeks[$i][$j + 1] . '","%Y-%m-%d")';
                            //exit();
                        }
                    } else if ($day == '14') {
                        ...
                        if ($current_date >= $weeks[$i][$j] && $current_date <= $weeks[$i][$j + 1]) {
                            ...
                            $query = 'SELECT * FROM ' . $table[0] . ' WHERE consigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2  AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $weeks[$i][$j] . '","%Y-%m-%d") - ' . $multiply_days . ' and str_to_date("' . $weeks[$i][$j + 1] . '","%Y-%m-%d")';
                        }
                    } else if ($day == '21') {
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            if ($count_files > 0) {
                ...
                $check_date_exist = mysqli_query($conn, "select * from `automation_mail_report` where consigner = '$consigner' and frequency_type = '7D' and client_type ='1'");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `created_at`, `updated_at`) VALUES('7D','1','$consigner','$consignees','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued','$current_date','$current_date')";
