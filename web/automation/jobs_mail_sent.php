...
switch($get_frq_type){
    case '1D':
        ...
        echo $query =  "SELECT * FROM `automation_mail_report` where frequency_type = '$get_frq_type' and job_status = 'Queued' and client_type = '1' ";
        break;
    ...
    case '7D':
       ...
       echo $query =  "SELECT * FROM `automation_mail_report` where frequency_type = '$get_frq_type' and job_status = 'Queued' and client_type = '1' ";
        ...
        break;
    case '15D':
         $query =  "SELECT * FROM `automation_mail_report` where frequency_type = '$get_frq_type' and job_status = 'Queued' and client_type = '1' ";
...
if (mysqli_num_rows($check_pending_jobs) > 0) {
    while ($row = mysqli_fetch_assoc($check_pending_jobs)) { //While Select First Row
        $id = $row['id'];
        $consigner = $row['consigner']; //First 
        $grn_no = explode(',', $row['grn_no']);
        $grn_date = explode(',', $row['grn_date']);
        $consignee = explode(',', $row['consignee']);
    ...
    //exit();
        ...
        if ($count_files > 0) {
            ...
            if (!empty(get_client_email($conn, $consigner))) {
                ...
                echo "Sent";
                 //exit();
                    $query_update = "UPDATE `automation_mail_report` SET job_status = 'SUCCESS', updated_at = '$current_date' where id = '$id'";
                        ...
                        if ($get_frq_type == '30D' || $get_frq_type == '45D' || $get_frq_type == '60D') {
                            ...
                            if ($count_grn_dates > 0) {
                                for ($h = 0; $h < $count_grn_dates; $h++) {
                                    ...
                                    // echo "<br>";
                                    // echo "<br>";
                                    echo $update_trans_table = "UPDATE " . $transaction . $m1 . "_" . $y . " SET `frq_sent_status`='SENT' WHERE consigner = '$consigner' and `transaction_id` = '$transaction_id'";
