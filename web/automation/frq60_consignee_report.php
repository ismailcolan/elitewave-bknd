...
if (mysqli_num_rows($check_frq) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($check_frq)) {
        ...
        echo $query = 'SELECT * FROM ' . $transaction . $mm . "_" . $yy . ' WHERE consignee = "' . $client_id . '" and mode_of_consignment = 1 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $first_day_of_month . '","%Y-%m-%d") and str_to_date("' . $last_day_of_month . '","%Y-%m-%d")
            ...
            UNION SELECT * FROM ' . $transaction . $mm2 . "_" . $yy2 . ' WHERE consigner = "' . $client_id . '" and status = 1 AND `book_manual` = 1 and frq_sent_status = "" and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $previous_month . '","%Y-%m-%d") and str_to_date("' . $previous_month_last_date . '","%Y-%m-%d")
            UNION SELECT * FROM ' . $transaction . $mm3 . "_" . $yy3 . ' WHERE consigner = "' . $client_id . '" and status = 1 AND `book_manual` = 1 and frq_sent_status = "" and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $next_fifteen_days_of_previous_month . '","%Y-%m-%d") and str_to_date("' . $last_date_of_fifteen_days_of_prev_month . '","%Y-%m-%d")
    ';
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            if ($count_files > 0) {
                ...
                $check_date_exist = mysqli_query($conn, "select * from `automation_mail_report` where consignee = '$consignee' and frequency_type = '60D' and client_type ='2'");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`,`client_type`, `consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `created_at`, `updated_at`) VALUES('60D','2','$consigners','$consignee','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued','$current_date','$current_date')";
