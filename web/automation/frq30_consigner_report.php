    ...
    while ($row = mysqli_fetch_assoc($check_frq)) {
        ...
        switch ($month) {
            case 1:
                ...
                UNION SELECT * FROM ' . $transaction . $m1 . "_" . $y . ' WHERE consigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $previous_month . '","%Y-%m-%d") and str_to_date("' . $previous_month_last_date . '","%Y-%m-%d")
                ';
                break;
            case 2:
                $previous_month = date('Y-m-d', strtotime($first_day_of_month . '-1 month'));
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            if ($count_files > 0) {
                ...
                $check_date_exist = mysqli_query($conn, "select * from `automation_mail_report` where consigner = '$consigner' and frequency_type = '30D' and client_type ='1'");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `created_at`, `updated_at`) VALUES('30D','1','$consigner','$consignees','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued','$current_date','$current_date')";
