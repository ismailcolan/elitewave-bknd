...
if (mysqli_num_rows($check_frq) > 0) {
    while ($row = mysqli_fetch_assoc($check_frq)) {
        ...
        $query = 'SELECT * FROM '.$table[0].' WHERE STR_TO_DATE(grn_date,"%d-%m-%Y") < STR_TO_DATE("'.$current_date.'","%d-%m-%Y") AND  consignee = '.$client_id.' AND  `mode_of_consignment` = 4 AND `status` = 8 AND `frq_sent_status` = "" AND (ISNULL(paid_status) OR `paid_status` = 0 OR `paid_status` = 2) AND `book_manual` = 1';
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            if ($count_files > 0) {
                $freq_type = $frq_day."D";
                $check_date_exist = mysqli_query($conn, "select * from `automation_mail_report` where consignee = '$consignee' and frequency_type = '$freq_type' and client_type ='4' AND `job_status` != 'SUCCESS'");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `frq_sent_date`, `created_at`, `updated_at`) VALUES('$freq_type','4','$consigners','$consignee','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued', '$current_date','$created_date','$created_date')";
        ...
        $query1 = 'SELECT * FROM '.$table_prev_quarter[0].' WHERE STR_TO_DATE(grn_date,"%d-%m-%Y") < STR_TO_DATE("'.$current_date.'","%d-%m-%Y") AND  consignee = '.$client_id.' AND  `mode_of_consignment` = 4 AND `status` = 8 AND `frq_sent_status` = "" AND (ISNULL(paid_status) OR `paid_status` = 0 OR `paid_status` = 2) AND `book_manual` = 1';
        ...
        if (mysqli_num_rows($transactions1) > 0) {
            ...
            if ($count_files1 > 0) {
                $freq_type = $frq_day."D";
                $check_date_exist1 = mysqli_query($conn, "select * from `automation_mail_report` where consignee = '$consignee1' and frequency_type = '$freq_type' and client_type ='4' AND `job_status` != 'SUCCESS'");
                $result_check1 = mysqli_num_rows($check_date_exist1);

                if ($result_check1 == 0) {
                    $queue_job1 = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `frq_sent_date`, `created_at`, `updated_at`) VALUES('$freq_type','4','$consigners1','$consignee1','$grn_numbers1','$grn_dates1','$origins1','$destinations1','$transct_id1','$amounts1','$invoice_no1','$invoice_filess1','Queued', '$current_date','$created_date','$created_date')";
