...
$current_date = '2022-05-05'; //CronSet Every 2 Month Gap [01,03,05,07,09,11]
...
$find_client = "select * from client where invoice_frequency = '60'"; // 1000 Entry
...
if (mysqli_num_rows($check_frq) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($check_frq)) {
        ...
        $first_day_of_month = date('Y-m-01', strtotime($current_date . '-2 Months')); // 01- 03-2022 Q1
        $last_day_of_month = date('Y-m-t', strtotime($current_date . '-2 Months')); // 31- 03-2022 
        ...
        $last_date_of_fifteen_days_of_month = date('Y-m-t', strtotime($next_fifteen_days_of_month)); //15-04-2022
        ...
        $next_fifteen_days_of_previous_month = date('Y-m-d', strtotime($previous_month . '-1 months')); //
        $last_date_of_fifteen_days_of_prev_month = date('Y-m-t', strtotime($next_fifteen_days_of_previous_month)); //
        ...
        echo $query = 'SELECT * FROM ' . $transaction . $mm . "_" . $yy . ' WHERE cconsigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $first_day_of_month . '","%Y-%m-%d") and str_to_date("' . $last_day_of_month . '","%Y-%m-%d")
            UNION SELECT * FROM ' . $transaction . $mm1 . "_" . $yy1 . ' WHERE cconsigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $next_fifteen_days_of_month . '","%Y-%m-%d") and str_to_date("' . $last_date_of_fifteen_days_of_month . '","%Y-%m-%d")
            UNION SELECT * FROM ' . $transaction . $mm2 . "_" . $yy2 . ' WHERE cconsigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $previous_month . '","%Y-%m-%d") and str_to_date("' . $previous_month_last_date . '","%Y-%m-%d")
            UNION SELECT * FROM ' . $transaction . $mm3 . "_" . $yy3 . ' WHERE cconsigner = "' . $client_id . '" and mode_of_consignment = 2 and status = 8 and frq_sent_status = "" and ISNULL(paid_status) or paid_status = 0 or paid_status = 2 AND `book_manual` = 1 and str_to_date(grn_date,"%d-%m-%Y") BETWEEN str_to_date("' . $next_fifteen_days_of_previous_month . '","%Y-%m-%d") and str_to_date("' . $last_date_of_fifteen_days_of_prev_month . '","%Y-%m-%d")
    ';
    ...
    // exit();
        ...
        if (mysqli_num_rows($transactions) > 0) {
            ...
            while ($row2 = mysqli_fetch_assoc($transactions)) {
                ...
                $find_invoice_file = array($consigner, $m1 . "_" . $y .  "_" . $row2['transaction_id'] . "invoice");
                // echo "<pre>";
            ...
            if ($count_files > 0) {
                ...
                $check_date_exist = mysqli_query($conn, "select * from `automation_mail_report` where consigner = '$consigner' and frequency_type = '60D' and client_type ='1' ");
                $result_check = mysqli_num_rows($check_date_exist);

                if ($result_check == 0) {
                    $queue_job = "INSERT INTO `automation_mail_report`(`frequency_type`, `client_type`,`consigner`, `consignee`, `grn_no`, `grn_date`, `origin`, `destination`,`transaction_id`, `invoice_amount`, `invoice_no`, `invoice_files`, `job_status`, `created_at`, `updated_at`) VALUES('60D','1','$consigner','$consignees','$grn_numbers','$grn_dates','$origins','$destinations','$transct_id','$amounts','$invoice_no','$invoice_filess','Queued','$current_date','$current_date')";
                    ...
                    if ($ress) {
                        ...
                        if ($count_grn_dates > 0) {
                            ...
                            for ($h = 0; $h < $count_grn_dates; $h++) {
                                ...
                                echo $update_trans_table = "UPDATE " . $transaction . $m1 . "_" . $y . " SET `frq_sent_status`='Queued' WHERE consigner = '$client_id' and `transaction_id` = '$transaction_id'";
