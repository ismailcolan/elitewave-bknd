...
<div class="container-fluid main-content new_dpt_bottom">
					...
					<div class="widget-content padded clearfix new_dept">
						<table class="table table-bordered table-striped" id="dataTable1">
							<thead>
								...
								<th class="table-title" style="width:30%">City Name</th>
								<th class="table-title" style="width:20%">State</th>
		...
		//Button Delete
			...
			$(document).on('click', '.btn-confirm-delete', function(ev){
				...
				$.post('save_details.php', { form_name: "del_city", tbl_id: $(this).attr("id") }, function(data,status){	
				console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
		...
		//Active Inactive
			$(document).on('click', '.btn-active', function(ev){
				...
				$.post('save_details.php', { form_name: "inacv_city", tbl_id: $(this).attr("id"),status:status1}, function(data,status){
					console.log(data);
					if(data == 1){
						$(".form-data-saving").hide();
						$("#alert-status").text("");
			...
			$(document).on('click', '.btn-edit', function(ev){
				...
				$.ajax({
					...
					type : "GET", // type of action POST || GET
					dataType : 'json', // data type
					data : { cmd: "get_city_details", tbl_id: tbl_id }, // post data || get data
					success : function(result) {
					console.log(result);
						$(".form-data-saving").hide();
