...
class Payments
{
    ...
    function SetOutStandingInfo($conn, $client_id, $amount)
    {
        ...
        if ($outstanding_count > 0) {
            ...
            $update_balance = (float)$upadate_total - (float)$upadate_amount_paid; //Update Balance Amount

            $update_outstanding = mysqli_query($conn, "UPDATE `client_outstanding` SET `total`='$upadate_total',`amount_paid`='$upadate_amount_paid',`balance`='$update_balance' WHERE client_id = '$c_id'");
        } else {
